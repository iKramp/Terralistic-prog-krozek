...
void BlockSelector::render() {
    ...
    int player_x = player_handler->getMainPlayer()->getX() / (BLOCK_WIDTH * 2);
    int player_y = player_handler->getMainPlayer()->getY() / (BLOCK_WIDTH * 2);
    ...
    if (std::abs(selected_block_x - player_x) <= 8 && std::abs(selected_block_y - player_y) <= 8) {
...
bool BlockSelector::onKeyDown(gfx::Key key) {
    if (key == gfx::Key::MOUSE_RIGHT) {
        int starting_x = player_handler->getMainPlayer()->getX() / (BLOCK_WIDTH * 2) - 8;
        int starting_y = player_handler->getMainPlayer()->getY() / (BLOCK_WIDTH * 2) - 8;
        int ending_x = player_handler->getMainPlayer()->getX() / (BLOCK_WIDTH * 2) + 8;
        int ending_y = player_handler->getMainPlayer()->getY() / (BLOCK_WIDTH * 2) + 8;
